...
def shift_activation_to_sym(q_model):
    ...
    with torch.no_grad():
        ...
        for name, module in q_model.named_modules():
            ...
            if 'ada_lin' in module_name and "shared_ada_lin" not in name and "head" not in name:
                logger.info(name)
                adaLN_module = module[1]
                next_module1 = father_module.attn.mat_qkv
                next_module2 = father_module.ffn.fc1
                ...
                add_bias = torch.cat([torch.full_like(ps1, 0), torch.full_like(ps2, 0), torch.full_like(ps1, 0), torch.full_like(ps2, 0), -ps1, -ps2])
                scale = torch.cat([torch.full_like(ps1, 1), torch.full_like(ps2, 1), s1, s2, s1, s2])
                ...
                adaLN_module.weight_quantizer.delta = None

            if "head_nm" in module_name:
                adaLN_module = module.ada_lin[1]
                m = father_module.head
                r, b, m.input_quantizer.delta, m.input_quantizer.zero_point = \
                    m.SmoothModule.init_sym_shift(m.input_quantizer.delta, m.input_quantizer.zero_point, m.input_quantizer.n_bits)

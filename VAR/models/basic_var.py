...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, attn_bias):
        ...
        if using_flash:
            ...
            attn_scores = self.matmul1(q, k.transpose(-2, -1))
            attn_scores = attn_scores * self.scale
            if attn_bias is not None:
                attn_scores = attn_scores + attn_bias  
            attn_weights = torch.softmax(attn_scores, dim=-1)
            oup = self.matmul2(attn_weights, v).transpose(1, 2).reshape(B, L, C)
...
class AdaLNSelfAttn(nn.Module):
    ...
    def forward(self, x, cond_BD, attn_bias):   # C: embed_dim, D: cond_dim
        ...
        x = x + self.drop_path(self.attn( self.ln_wo_grad(x).mul(scale1.add(self.original_constant1)).add_(shift1), attn_bias=attn_bias).mul_(gamma1))
        x = x + self.drop_path(self.ffn( self.ln_wo_grad(x).mul(scale2.add(self.original_constant2)).add_(shift2) ).mul(gamma2)) # this mul(gamma2) cannot be in-placed when FusedMLP is used
        return x
...
class AdaLNBeforeHead(nn.Module):
    ...
    def forward(self, x_BLC: torch.Tensor, cond_BD: torch.Tensor):
        ...
        return self.ln_wo_grad(x_BLC).mul(scale.add(self.original_constant)).add_(shift)

...
class Attention(nn.Module):
    ...
    def forward(
        ...
        d_k = xq.shape[-1]  
        scores = self.matmul1(xq, keys.transpose(-2, -1)) / d_k**0.5  
        if mask is not None:
            scores = scores.masked_fill(mask == False, float('-inf'))
        attn_weights = F.softmax(scores, dim=-1)
        output = self.matmul2(attn_weights, values)  
...
##################################################################################################################################################################
        ...
        # globalvar.appendInput((h.clone().cpu(), freqs_cis.clone().cpu(), input_pos.clone().cpu(), mask.clone().cpu(),))
##################################################################################################################################################################
    ...
    def forward_fn(self, h, freqs_cis, input_pos, mask):
        # transformer blocks
        for layer in self.layers:
            h = layer(h, freqs_cis, input_pos, mask)
        h = self.norm(h)

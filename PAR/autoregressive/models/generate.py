...
def prefill(model, cond_idx: torch.Tensor, input_pos: torch.Tensor, cfg_scale: float, **sampling_kwargs):
    # set current_token for wo and w2
    from quant.quantizer import TokenAwareUniformQuantizer
    for name, m in model.named_modules():
        # if ('wo' in name or "w2" in name) and isinstance(m, (TokenAwareUniformQuantizer)):
        if isinstance(m, (TokenAwareUniformQuantizer)):
            m.current_token = input_pos.item()
...
def decode_one_token_multi(model, x: torch.Tensor, input_pos: torch.Tensor, cfg_scale: float, cfg_flag: bool, spe_token_num: int, **sampling_kwargs):
    ...
    for name, m in model.named_modules():
        # if ('wo' in name or "w2" in name) and isinstance(m, (TokenAwareUniformQuantizer)):
        if isinstance(m, (TokenAwareUniformQuantizer)):
            m.current_token = input_pos[0].item()

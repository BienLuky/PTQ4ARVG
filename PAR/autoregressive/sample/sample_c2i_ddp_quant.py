...
def main(args):
    ...
    logdir = os.path.join(args.output, "log", now+'-PAR_qunat')
    os.makedirs(logdir)
    ...
    q_model = quant_model(gpt_model, input_quant_params=aq_params, weight_quant_params=wq_params)
    q_model.to(device)
    q_model.eval()

    hs, freqs_cis, input_pos, mask = torch.load(f"{args.output}/cali_{gpt}_128_dgc.pth")
    ...
    set_smooth_quantize_params(q_model, hs, freqs_cis, input_pos, mask, batch_size=batch)

    logger.info("Performing initial quantization ...")
    set_weight_quantize_params(q_model, hs, freqs_cis, input_pos, mask)
    set_act_quantize_params(q_model, hs, freqs_cis, input_pos, mask, batch_size=batch)
    ...
    reset_weight_quantize_params(q_model, hs, freqs_cis, input_pos, mask)
    reset_act_quantize_params(q_model, hs, freqs_cis, input_pos, mask, batch_size=batch)

    logger.info(75 * "*")
    logger.info("calculating optimal factor")
    set_calculating_optimal_factor(q_model, hs, freqs_cis, input_pos, mask, batch_size=batch)

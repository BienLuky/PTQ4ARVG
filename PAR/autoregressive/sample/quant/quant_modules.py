...
from .quantizer import UniformQuantizer, LogSqrt2Quantizer, TokenAwareUniformQuantizer
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'SmoothModule'):
            if self.SmoothModule.inited == False:
                ...
                if self.sink_aware:
                    self.SmoothModule.init_smooth_scale(x[:,8:,:], self.ori_weight, self.input_quantizer, self.weight_quantizer)
                else:
        ...
        if self.use_input_quant:
            if self.sink_aware:
                sink_x = x[:,:8,:]
                other_x = self.input_quantizer(x[:,8:,:])

...
class SmoothModule(nn.Module):
    ...
    def calculate_scales_hessian(self, input, weight, alpha, input_quantizer, weight_quantizer):
        ...
        for i in range(len(scales)):
            if i != idx:
                ...
                frac_22 = torch.sum(torch.square(q_weight[:,i])) 
    ...
    def calculate_sym_shift(self, input_s, input_zp, bit_width: int):
    ...
    def init_smooth_scale(self, input, weight, input_quantizer=None, weight_quantizer=None):
        if self.calculate_optimal_factor:
            '''GPS'''
            scales, shifts = self.calculate_scales_hessian(input, weight, alpha=0.5, input_quantizer=input_quantizer, weight_quantizer=weight_quantizer)
        else:
            '''smoothquant'''
            scales, shifts = self.calculate_scales(input, weight, alpha=0.5)
        self.scales, self.shifts = self.update_params(scales, shifts)
        return self.scales, self.shifts

    def update_params(self, scales, shifts):
        if self.scales is None:
            self.scales = scales
            self.shifts = shifts
        self.scales = 0.1 * scales + 0.9 * self.scales
        self.shifts = 0.1 * shifts + 0.9 * self.shifts
        return self.scales, self.shifts
...
class UniformQuantizer(nn.Module):
    ...
    def init_quantization_scale(self, x: torch.Tensor, channel_wise: bool = False):
        ...
        if channel_wise:
            ...
            x_clone = x.clone().detach().to(dtype=torch.float32)
            x_max = x_clone.max()
            x_min = x_clone.min()
            best_score = 1e+10
...
class LogSqrt2Quantizer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.inited is False:
            ...
            self.delta = self.update_params(delta)
            # self.inited = True
...
class TokenAwareUniformQuantizer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        if self.inited is False:
            if self.channel_wise:
                ...
                delta = [None] * 6
                zero_point = [None] * 6
                delta[0], zero_point[0] = self.init_quantization_scale(x[:,0,:], self.channel_wise)
                delta[1], zero_point[1] = self.init_quantization_scale(x[:,1,:], self.channel_wise)
                delta[2], zero_point[2] = self.init_quantization_scale(x[:,2,:], self.channel_wise)
                delta[3], zero_point[3] = self.init_quantization_scale(x[:,3,:], self.channel_wise)
                delta[4], zero_point[4] = self.init_quantization_scale(x[:,4:8,:], self.channel_wise)
                delta[5], zero_point[5] = self.init_quantization_scale(x[:,8:,:], self.channel_wise)
                delta, zero_point = torch.stack(delta).to(dtype=torch.bfloat16), torch.stack(zero_point).to(dtype=torch.bfloat16)
            self.delta, self.zero_point = self.update_params(delta, zero_point)

        # start quantization
        token_num = x.size(1)
        if token_num == 576: # cali
            delta = torch.cat([self.delta[0].view(1), self.delta[1].view(1), self.delta[2].view(1), self.delta[3].view(1), \
                self.delta[4].repeat(4), self.delta[5].repeat(token_num-8)])
            zero_point = torch.cat([self.zero_point[0].view(1), self.zero_point[1].view(1), self.zero_point[2].view(1), self.zero_point[3].view(1), \
                self.zero_point[4].repeat(4), self.zero_point[5].repeat(token_num-8)])
        else: # infer
    ...
    def init_quantization_scale(self, x: torch.Tensor, channel_wise: bool = False):
        ...
        if channel_wise:
            ...
            x_clone = x.clone().detach().to(torch.float32)
            x_max = x_clone.max()
            x_min = x_clone.min()
            best_score = 1e+10
            for pct in [0.999, 0.9999, 0.99999]:

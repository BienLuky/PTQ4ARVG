'''
First, remember to uncomment line 438-439 in ./autoregressive/models/gpt.py and comment them after finish collecting.
'''
import torch
...
def main(args):
    ...
    logdir = os.path.join(args.output, "log", now+'-PAR_cali')
    os.makedirs(logdir)
    ...
    for i in range(int(number/batch)):
        c_indices = torch.randint(0, args.num_classes, (batch,), device=device)
    ...
    for i in range(len(input_list[0])):
        h = torch.cat([inp[i][0] for inp in input_list])
        hs.append(h)
    freqs_cis = torch.cat([sub_inp[1] for sub_inp in input_list[0]])
    input_pos = torch.cat([sub_inp[2] for sub_inp in input_list[0]])
    mask = torch.cat([sub_inp[3] for sub_inp in input_list[0]], dim=2)
    mask = mask[:,:,:,:mask.size(2)]
    mask = torch.cat([mask]*(int(number/batch)), dim=0)
    torch.save((hs, freqs_cis, input_pos, mask), f"{args.output}/cali_{gpt}_{number}.pth")

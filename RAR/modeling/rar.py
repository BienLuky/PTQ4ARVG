...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask=None) -> torch.Tensor:
        ...
        attn_scores = self.matmul1(q, k.transpose(-2, -1))  # (batch_size, num_heads, seq_len_q, seq_len_k)
        d_k = q.size(-1)  # d_k is the dimension of query/key
        attn_scores = attn_scores / torch.sqrt(torch.tensor(d_k, dtype=torch.float32))
        if attn_mask is not None:
            attn_scores = attn_scores + attn_mask  
        attn_weights = torch.softmax(attn_scores, dim=-1)  # (batch_size, num_heads, seq_len_q, seq_len_k)
        x = self.matmul2(attn_weights, v)  # (batch_size, num_heads, seq_len_q, d_v)
...
class FinalLayer(nn.Module):
    def __init__(self, dim, norm_layer):
        ...
        self.original_constant = 1
    
    def absorb_modulate(self, x, shift, scale):
        return x * (self.original_constant + scale) + shift

    def forward(self, x, c):
        scale, shift = self.adaLN_modulation(c).chunk(2, dim=-1)
        x = self.absorb_modulate(self.norm_final(x), shift, scale)
        return x
...
class Block(nn.Module):
    def __init__(
        ...
        self.original_constant1 = 1
        self.original_constant2 = 1

    def absorb_modulate1(self, x, shift, scale):
        return x * (self.original_constant1 + scale) + shift
    def absorb_modulate2(self, x, shift, scale):
        return x * (self.original_constant2 + scale) + shift

    def forward(self, x: torch.Tensor, attn_mask=None, c = None) -> torch.Tensor:
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).chunk(6, dim=-1)
        x = x + gate_msa * self.attn(self.absorb_modulate1(self.norm1(x), shift_msa, scale_msa), attn_mask=attn_mask)
        x = x + gate_mlp * self.mlp(self.absorb_modulate2(self.norm2(x), shift_mlp, scale_mlp))
        return x
...
class RAR(BaseModel):
    ...
    def forward_fn(self, input_ids, condition,
        ...
        # set current_token for adaln module
        from quant.quantizer import TokenAwareUniformQuantizer
        for name, m in self.named_modules():
            if 'adaLN_modulation' in name and "adaln_before_head" not in name:
                if isinstance(m, (TokenAwareUniformQuantizer)):
                    m.current_token = x.shape[1] - 1
            elif 'adaLN_modulation' in name and "adaln_before_head" in name:
                if isinstance(m, (TokenAwareUniformQuantizer)):
                    m.current_token = x.shape[1] - 2
...
#####################################################################################################################################
                # cali remove "##"
                # if step == self.image_seq_len-1:
                #     import quant.globalvar as globalvar
                #     globalvar.appendInput((torch.cat([ids, ids], dim=0),
                #                             torch.cat([condition, self.get_none_condition(condition)], dim=0),))
#####################################################################################################################################

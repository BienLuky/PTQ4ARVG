...
def set_smooth_quantize_params(q_model, input_ids, condition):
...
def set_calculating_optimal_factor(q_model, input_ids, condition):
    logger.info(f"set_calculating_optimal_factor")

    set_quant_state(q_model, input_quant=False, weight_quant=False)
    for name, module in q_model.named_modules():
        if isinstance(module, QuantLinear) and hasattr(module, 'SmoothModule') and "head" not in name:
            module.SmoothModule.inited = False

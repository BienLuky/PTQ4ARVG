    ...
    def forward(self, x):
        if hasattr(self, 'SmoothModule'):
            if self.SmoothModule.inited == False:
                # sink tokens are quantized separately, so scaling does not consider sink tokens
                if self.sink_aware:
                    self.SmoothModule.init_smooth_scale(x[:,2:,:], self.weight, self.input_quantizer, self.weight_quantizer)
                else:
        ...
        if self.use_input_quant:
            if self.sink_aware:
                sink_x = x[:,:2,:]
                other_x = self.input_quantizer(x[:,2:,:])

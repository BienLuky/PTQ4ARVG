...
from modeling.rar import MatMul
...
def quant_model(model, input_quant_params={}, weight_quant_params={}):
    ...
    for name, m in model.named_modules():
        ...
        if isinstance(m, nn.Linear):
            ...
            elif 'qkv' in name or 'fc1' in name:
                new_m = QuantLinear(m.in_features, m.out_features, input_quant_params_channel, weight_quant_params, smooth=True)
                new_m.sink_aware = True

            elif "adaln_before_head" in name or "adaLN_modulation" in name:
                new_m = QuantLinear(m.in_features, m.out_features, input_quant_params_channel, weight_quant_params, token_aware=True)

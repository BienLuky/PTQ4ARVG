        ...
        if channel_wise:
            ...
            for pct in [0.999, 0.9999, 0.99999]:
                try:
                    ...
                    if torch.isnan(new_max) or torch.isnan(new_min):
                        new_max = torch.tensor(np.percentile(x_clone.reshape(-1).cpu().numpy(), pct * 100),
                            device=x_clone.device,
                            dtype=torch.float32)
                        new_min = torch.tensor(np.percentile(x_clone.reshape(-1).cpu().numpy(), (1 - pct) * 100),
                            device=x_clone.device,
                            dtype=torch.float32)   
                except:
                ...
                if torch.isnan(new_max) or torch.isnan(new_min):
                    raise ValueError("Quantile result is NaN: check input tensor or pct value.")
...
class TokenAwareUniformQuantizer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        if self.inited is False:
            if self.channel_wise:
                ...
                self.delta[0], self.zero_point[0] = self.init_quantization_scale(x[:,:64,:], True)
                self.delta[1], self.zero_point[1] = self.init_quantization_scale(x[:,64:,:], self.channel_wise)
            self.inited = True
        ...
        if token_num > 64:
            if self.channel_wise:
                ...
                delta = torch.cat([self.delta[0], self.delta[1].repeat(token_num-64)])
                zero_point = torch.cat([self.zero_point[0], self.zero_point[1].repeat(token_num-64)])
        else:
            delta = self.delta[0]
    ...
    def init_quantization_scale(self, x: torch.Tensor, channel_wise: bool = False):
        ...
        if channel_wise:
            ...
            x_max = x_clone.max()
            x_min = x_clone.min()
            if torch.isnan(x_clone).any():
                raise ValueError("zero Quantile result is NaN: check input tensor or pct value.")
            if x_max == x_min:
                print(x_max)
            assert x_max != x_min
            best_score = 1e+10
            for pct in [0.999, 0.9999, 0.99999]:
                try:
                    ...
                    if torch.isnan(new_max) or torch.isnan(new_min):
                        print("First Quantile result is NaN: check input tensor or pct value.")
                        new_max = torch.tensor(np.percentile(x_clone.reshape(-1).cpu().numpy(), pct * 100),
                            device=x_clone.device,
                            dtype=torch.float32)
                        new_min = torch.tensor(np.percentile(x_clone.reshape(-1).cpu().numpy(), (1 - pct) * 100),
                            ...
                            dtype=torch.float32)   
                    if torch.isnan(new_max) or torch.isnan(new_min):
                        raise ValueError("First Quantile result is NaN: check input tensor or pct value.")
                except:
                    new_max = torch.tensor(np.percentile(x_clone.reshape(-1).cpu().numpy(), pct * 100),
                        device=x_clone.device,
                        dtype=torch.float32)
                    new_min = torch.tensor(np.percentile(x_clone.reshape(-1).cpu().numpy(), (1 - pct) * 100),
                        ...
                        dtype=torch.float32)   
                    if torch.isnan(new_max) or torch.isnan(new_min):
                        raise ValueError("Second Quantile result is NaN: check input tensor or pct value.")
                x_q = self.quantize(x_clone, new_max, new_min)
                score = lp_loss(x_clone, x_q, p=2, reduction='all')
                if score < best_score:
                    best_score = score
